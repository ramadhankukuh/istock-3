...
import type { CandlePoint, StockResponse } from "@/features/stock-analysis/types";
...
export function normalizeStockCode(rawCode: string) {
...
export async function fetchStockData(rawCode: string): Promise<StockResponse> {
  ...
  const candles: CandlePoint[] = history
    .filter(
      (d: any) =>
        d?.date &&
        d?.open !== null &&
